namespace BeejaServer.Services
{
    public static class LevelService
    {
        ...
        private const double Multiplier = 1.5; // множитель прогресса до 5 уровня
        ...
        public static int GetRequiredPointsForLevel(int level)
        {
            ...
            for (int i = 1; i < level; i++)
            {
                ...
                // До 5 уровня используем геометрическую прогрессию (умножение на 1.5)
                // Начиная с 5 уровня применяем формулу: 1.1 * x + 200 (где x — текущая стоимость)
                if (i < 5)
                {
                ...
                {
                    currentLevelCost = (currentLevelCost * 1.1) + 200;
                }
            }
